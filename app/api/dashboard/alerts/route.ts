...
export async function GET() {
  try {
    ...
    const [lowStock, upcoming, payments, salaryDue] = await Promise.all([
      query<{ name: string; available_quantity: number; low_stock_threshold: number }>(
      ...
      query<{ event_name: string; event_date: string; customer_name: string }>(
      ...
      query<{ customer_name: string; remaining_balance: string }>(
